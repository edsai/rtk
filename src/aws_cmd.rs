...
/// Generic strategy: force --output json, compress via json_cmd schema
fn run_generic(subcommand: &str, args: &[String], verbose: u8, full_sub: &str) -> Result<()> {
    ...
    // Force JSON output if user specified table/text or no output flag
    if !has_output_flag {
        ...
        for i in 1..=10 {
            instances.push(format!(
                r#"{{
                    ...
                    "InstanceType": "t3.micro",
                    "PrivateIpAddress": "10.0.1.{}",
                    "PublicIpAddress": "54.0.0.{}",

...
";
    ...
    fn test_table_token_savings() {
        let mut lines = vec![
            " id         | name       | email              | status    ".to_string(),
            "------------+------------+--------------------+-----------".to_string(),
        ];
        for i in 1..=20 {
            lines.push(format!(
        ...
        assert!(
            savings >= 30.0,
            "Table filter: expected >=30% savings, got {:.1}%",
            savings
        );
    }
    ...
    fn test_expanded_token_savings() {
        ...
        for i in 1..=5 {
            ...
            lines.push(format!("id         | {}", i));
            lines.push(format!("name       | user{}", i));
            lines.push(format!("email      | user{}@example.com", i));
            lines.push("status     | active".to_string());
            lines.push(format!("created_at | 2024-01-{:02} 10:00:00", i));
        }
        lines.push("(5 rows)".to_string());
        let input = lines.join("\n");
